...
type
  TREL_T1_Frame = class(TTradeStrategyFrame)
    Panel1: TPanel;
    GroupBox2: TGroupBox;
    Label3: TLabel;
    EditLENGTH1: TEdit;
...
//---------------------------------------------------------------------------
constructor TREL_T1_Frame.Create(AOwner: TComponent) ;
begin
...
//---------------------------------------------------------------------------
procedure TREL_T1_Frame.MakeDefaultOption(AOption: CMXTradeStrategyOption);
begin
    CMXTradeStrategyOption.Default_REL_T1_O2(AOption);
end;

//---------------------------------------------------------------------------
//  화면컨트롤에서 데이터를 가져온다.
procedure TREL_T1_Frame.GetControlData;
begin
    ...
    m_Option.SetDoubleValue('UP', TFNGlobal.atof(EditUP.Text));
    m_Option.SetDoubleValue('DN', TFNGlobal.atof(EditDN.Text));
    m_Option.SetIntegerValue('METHOD', ComboBoxMETHOD.ItemIndex);
    m_Option.SetBooleanValue('USEEXIT', CheckBoxUSEEXIT.Checked);

end;

//---------------------------------------------------------------------------
procedure TREL_T1_Frame.SetControlData;
begin
    ...
    try
        ...
        EditUP.Text := FloatToStr(m_Option.GetDoubleValue('UP'));
        EditDN.Text := FloatToStr(m_Option.GetDoubleValue('DN'));
        ComboBoxMETHOD.ItemIndex := m_Option.GetIntegerValue('METHOD');
        CheckBoxUSEEXIT.Checked := m_Option.GetBooleanValue('USEEXIT');
    finally
        m_EnableEvent := true;
    end;
end;
...
//---------------------------------------------------------------------------
procedure TREL_T1_Frame.ButtonApplyTSOptionClick(Sender: TObject);
begin
    ApplyOptionCollection;
end;

//---------------------------------------------------------------------------
procedure TREL_T1_Frame.ButtonFavorClick(Sender: TObject);
begin
    AddFavorOptionCollection;
end;

//---------------------------------------------------------------------------
procedure TREL_T1_Frame.ButtonOptionManagementClick(Sender: TObject);
begin
    ManagementOptionCollection;
end;

//---------------------------------------------------------------------------
procedure TREL_T1_Frame.ButtonPopupMenuClick(Sender: TObject);
var
    f_PT:TPoint;
begin
...
//---------------------------------------------------------------------------
procedure TREL_T1_Frame.ComboBoxMajorTypeChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
...
//---------------------------------------------------------------------------
procedure TREL_T1_Frame.ComboBoxOptionCollectionChange(Sender: TObject);
begin
    ApplyOptionCollection;
end;

//---------------------------------------------------------------------------
procedure TREL_T1_Frame.EditKeyPress(Sender: TObject; var Key: Char);
begin
    if Key in [#$D] then
    begin
        OptionChange(Sender);
...
//---------------------------------------------------------------------------
procedure TREL_T1_Frame.UpdateControlData;
begin
    SetControlData;
end;

//---------------------------------------------------------------------------
procedure TREL_T1_Frame.UpDownClick(Sender: TObject; Button: TUDBtnType);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
...
//---------------------------------------------------------------------------
procedure TREL_T1_Frame.EditChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
...
//---------------------------------------------------------------------------
procedure TREL_T1_Frame.OptionChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
