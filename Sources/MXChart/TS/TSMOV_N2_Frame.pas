...
type
  TMOV_N2_Frame = class(TTradeStrategyFrame)
    Panel1: TPanel;
    Panel3: TPanel;
...
constructor TMOV_N2_Frame.Create(AOwner: TComponent) ;
begin
...
procedure TMOV_N2_Frame.MakeDefaultOption(AOption: CMXTradeStrategyOption);
begin
    CMXTradeStrategyOption.Default_MOV_N2_O1(AOption);
end;

//  화면컨트롤에서 데이터를 가져온다.
procedure TMOV_N2_Frame.GetControlData;
begin
...
procedure TMOV_N2_Frame.SetControlData;
var
    f_NValue : Integer;
begin
...
procedure TMOV_N2_Frame.ButtonApplyTSOptionClick(Sender: TObject);
begin
    ApplyOptionCollection;
end;

procedure TMOV_N2_Frame.ButtonFavorClick(Sender: TObject);
begin
    AddFavorOptionCollection;
end;


procedure TMOV_N2_Frame.ButtonOptionManagementClick(Sender: TObject);
begin
    ManagementOptionCollection;
end;

procedure TMOV_N2_Frame.ButtonPopupMenuClick(Sender: TObject);
var
    f_PT:TPoint;
begin
...
procedure TMOV_N2_Frame.ComboBoxMajorTypeChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
...
procedure TMOV_N2_Frame.ComboBoxOptionCollectionChange(Sender: TObject);
begin
    ApplyOptionCollection;
end;

procedure TMOV_N2_Frame.EditKeyPress(Sender: TObject; var Key: Char);
begin
    if Key in  [#$D] then
    begin
        EditChange(Sender);
...
procedure TMOV_N2_Frame.UpdateControlData;
begin
    SetControlData;
end;

procedure TMOV_N2_Frame.UpDownClick(Sender: TObject; Button: TUDBtnType);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
...
procedure TMOV_N2_Frame.EditChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
...
procedure TMOV_N2_Frame.OptionChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
