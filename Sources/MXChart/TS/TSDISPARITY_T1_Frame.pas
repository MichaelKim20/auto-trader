...
type
  TDISPARITY_T1_Frame = class(TTradeStrategyFrame)
    Panel1: TPanel;
    GroupBox2: TGroupBox;
    Label3: TLabel;
    Label4: TLabel;
...
constructor TDISPARITY_T1_Frame.Create(AOwner: TComponent) ;
begin
...
procedure TDISPARITY_T1_Frame.MakeDefaultOption(AOption: CMXTradeStrategyOption);
begin
    CMXTradeStrategyOption.Default_DISPARITY_T1(AOption);
end;

//  화면컨트롤에서 데이터를 가져온다.
procedure TDISPARITY_T1_Frame.GetControlData;
begin
...
procedure TDISPARITY_T1_Frame.SetControlData;
begin
...
procedure TDISPARITY_T1_Frame.ButtonApplyTSOptionClick(Sender: TObject);
begin
    ApplyOptionCollection;
end;

procedure TDISPARITY_T1_Frame.ButtonFavorClick(Sender: TObject);
begin
    AddFavorOptionCollection;
end;


procedure TDISPARITY_T1_Frame.ButtonOptionManagementClick(Sender: TObject);
begin
    ManagementOptionCollection;
end;

procedure TDISPARITY_T1_Frame.ButtonPopupMenuClick(Sender: TObject);
var
    f_PT:TPoint;
begin
...
procedure TDISPARITY_T1_Frame.ComboBoxMajorTypeChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
...
procedure TDISPARITY_T1_Frame.ComboBoxOptionCollectionChange(Sender: TObject);
begin
    ApplyOptionCollection;
end;

procedure TDISPARITY_T1_Frame.EditKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TDISPARITY_T1_Frame.UpdateControlData;
begin
    SetControlData;
end;

procedure TDISPARITY_T1_Frame.UpDownClick(Sender: TObject; Button: TUDBtnType);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
...
procedure TDISPARITY_T1_Frame.EditChange(Sender: TObject);
begin
    if not Assigned(m_Option) then exit;
    if not m_EnableEvent then exit;
