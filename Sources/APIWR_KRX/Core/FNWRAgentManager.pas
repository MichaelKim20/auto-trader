...
type
    ...
    CFNWRAgentManager = class(CFNAgentManager)
    public
        ///<Comment>생성자</Comment>
...
uses
    FNGlobal, FNGlobalVariable, WideStrUtils, CommonTRMaker, FNTradeSystem, WRIOMaker, FNCMVariable, MXVariable;

//---------------------------------------------------------------------------
procedure CFNWRAgentManager.AssignAgent(p_Agent: TWRAX);
begin
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.AssignHandle(p_Handle: HWND);
begin
    m_Handle := p_Handle;
end;

//---------------------------------------------------------------------------
Constructor CFNWRAgentManager.Create(AQueryThreadCount:Integer = 1);
begin
...
//---------------------------------------------------------------------------
Destructor CFNWRAgentManager.Destroy;
begin
    m_DataStream.Free;
    m_StringStream.Free;
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.SetSBID(AMapType:Integer; AKey:String; AValue:Integer);
var
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.ClearSBID(AMapType:Integer; AKey:String);
var
    f_Index:Integer;
...
//---------------------------------------------------------------------------
function CFNWRAgentManager.GetSBID(AMapType:Integer; AKey:String):Integer;
var
    f_Index:Integer;
    f_Value:CFNFieldValue;
begin
...
//---------------------------------------------------------------------------
//  선물 스트리밍 시세 등록
procedure CFNWRAgentManager.SubscribeQuote(ADataDelivery: CFNDataDelivery; ACountry:Integer; AGroup:Integer; AMarket:Integer; ASymbol: String);
var
...
//---------------------------------------------------------------------------
//  선물 스트리밍 호가 등록
procedure CFNWRAgentManager.SubscribeBidOffer(ADataDelivery: CFNDataDelivery; ACountry:Integer; AGroup:Integer; AMarket:Integer; ASymbol: String);
var
    f_SubscribeKey : String;
    f_SBID:Integer;
begin
...
//---------------------------------------------------------------------------
//  선물 스트리밍 체결통보 등록
procedure CFNWRAgentManager.SubscribeUserTrade(ADataDelivery: CFNDataDelivery; AUserID: String; AAccountNO: String = ''; AAccountPW: String = '');
var
    f_SBID:Integer;
begin
    if not Assigned(m_Agent) then exit;
...
//---------------------------------------------------------------------------
//  선물 스트리밍 시세 등록 해지
procedure CFNWRAgentManager.UnsubscribeQuote(ADataDelivery: CFNDataDelivery; ACountry:Integer; AGroup:Integer; AMarket:Integer; ASymbol: String);
var
    f_SubscribeKey : String;
    f_SBID:Integer;
begin
...
//---------------------------------------------------------------------------
//  선물 스트리밍 호가 등록 해지
procedure CFNWRAgentManager.UnsubscribeBidOffer(ADataDelivery: CFNDataDelivery; ACountry:Integer; AGroup:Integer; AMarket:Integer; ASymbol: String);
var
    f_SubscribeKey : String;
    f_SBID:Integer;
begin
...
//---------------------------------------------------------------------------
//  선물 스트리밍 체결통보 등록 해지
procedure CFNWRAgentManager.UnsubscribeUserTrade(ADataDelivery: CFNDataDelivery; AUserID: String; AAccountNO: String = ''; AAccountPW: String = '');
var
    f_SBID:Integer;
begin
    if not Assigned(m_Agent) then exit;
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.ReSubscribeAll;
var
...
procedure CFNWRAgentManager.DoQueryWork(AThreadIndex:Integer);
var
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.DoCheckTimeout;
var
    f_Index:Integer;
    f_QueryData:CFNQueryData;
...
procedure CFNWRAgentManager.WriteMessage(p_QueryData: CFNQueryData; sErrorCode, sMsg: WideString);
var
    rd : CFNRecord;
begin
...
//---------------------------------------------------------------------------
//  패킷중 에러코드와 메세지를 추가하는 부분
procedure CFNWRAgentManager.WriteError(p_QueryData: CFNQueryData; sErrorCode:WideString);
var
    rd : CFNRecord;
begin
    p_QueryData.m_Response.SetErrorCode(sErrorCode);
...
procedure CFNWRAgentManager.SC_ORDER_TR_0210(p_QueryData: CFNQueryData);
var
    ...
    try
        ...
        begin
            ...
            begin
                ...
                begin
                    p_QueryData.m_RequestID := m_Agent.SendNewOrder(f_AccountNo, f_Password, f_Symbol, f_nBuySell, f_nPriceType, 1, f_OrderVolume, f_OrderPrice, 1);
                end else
                if (f_OrderCommand = 3) then
                begin
                    p_QueryData.m_RequestID := m_Agent.SendModiOrder(f_AccountNo, f_Password, f_Symbol, f_nBuySell, f_nPriceType, 1, f_OrderVolume, f_OrderPrice, f_Record.GetStringValue('ORG_ORDER_NO'), 1);
                end else
                if (f_OrderCommand = 4) then
                begin
                    p_QueryData.m_RequestID := m_Agent.SendCancelOrder(f_AccountNo, f_Password, f_Symbol, f_nBuySell, f_OrderVolume, f_Record.GetStringValue('ORG_ORDER_NO'), 1);
                end;
...
procedure CFNWRAgentManager.SC_QUOTE_TR_0210(p_QueryData: CFNQueryData);
var
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.SC_ACCOUNT_TR_0010(p_QueryData: CFNQueryData);
var
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.MakeDefaultResponse(p_QueryData:CFNQueryData);
var
    f_OutDataSet : CFNDataSet;
    f_OutRecord : CFNRecord;
    f_InDataSet : CFNDataSet;
...
procedure CFNWRAgentManager.OnTimeout(p_QueryData:CFNQueryData);
var
    f_QueryData:CFNQueryData;
    f_OutDataSet : CFNDataSet;
    f_OutRecord : CFNRecord;
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.OnRecvData(
ASender: TObject;
DataType: Smallint;
            ...
            begin
                ...
                try
                    ...
                    begin
                        ...
                        begin
                            LOG_WRITE(LOG_TYPE_ERROR, 'CFNWRAgentManager',
                            '주문응답오류; ' +
                            '사용자:'           +   f_OutRecord.GetStringValue('USERID')               + '; ' +
                            '계좌:'             +   f_OutRecord.GetStringValue('ACCOUNT_NO')           + '; ' +
                            '종목:'             +   f_OutRecord.GetStringValue('SYMBOL')               + '; ' +
...
{$REGION '실시간 데이터 수신'}
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.OnRecvRealData(ASender: TObject; const TrCode, KeyValue: WideString; RealID: Smallint; DataSize: Integer; const szData: WideString);
begin
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.Process_SB_FUT_EXEC(AKeyValue: WideString; ARealID: Smallint; ADataStream : TMemoryStream);
var
...
begin
    ...
    begin
        try
            ...
            f_SubscribeKey := MakeStreamSubscribeKey(0, 4, 0, f_Record.GetStringValue('SYMBOL'));
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.Process_SB_ORDER_EXEC(AKeyValue: WideString; ARealID: Smallint; ADataStream : TMemoryStream);
var
    f_StreamRecord:CFNStreamRecord;
...
begin
    ...
    try
        ...
        LOG_WRITE(LOG_TYPE_ERROR, 'CFNWRAgentManager', '패킷분석 실패');
    end;
    ...
    begin    
        try
            ...
            begin
                ...
                f_StreamRecord.SetDoubleValue('ORDER_PRICE'     , TFNGlobal.atof(f_SValue));
                ...
                f_SValue := f_Record.GetStringValue('ORDPX');
                f_StreamRecord2.SetDoubleValue('ORDER_PRICE'     , TFNGlobal.atof(f_SValue));
...
//---------------------------------------------------------------------------
procedure CFNWRAgentManager.OnNetConnected(ASender: TObject);
begin

end;

//---------------------------------------------------------------------------
procedure CFNWRAgentManager.OnNetDisconnected(ASender: TObject);
begin
    LOG_WRITE(LOG_TYPE_ERROR, 'CFNWRAgentManager', '통신이 종료되었습니다.');
