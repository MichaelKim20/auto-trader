...
procedure TExit_Frame.ApplyLanguage;
begin
  if (g_Language = 0) then
  begin
    Panel5.Caption := '청산';
    GroupBoxOptionManagement.Caption := '저장과 적용';
    GroupBox3.Caption := '손절청산';
    GroupBox1.Caption := '이익청산';
    GroupBox2.Caption := 'Trailing Stop';

    CheckBoxUSE_LOSSCUT.Caption := '사용여부';
    Label1.Caption := '진입가의 -X% 이하 손실이 발생할 때';

    GroupBox1.Caption := '이익청산';
    CheckBoxUSE_PROFITCUT.Caption := '사용여부';
    Label3.Caption := '진입가의 X% 이상 이익이 발생할 때';

    CheckBoxUSE_TRAILINGSTOP.Caption := '사용여부';
    Label5.Caption := '목표가 : 진입가의 X% ';
    Label7.Caption := '청산가격 : 최대수익의 Y% 감소';
  end
  else
  begin
    Panel5.Caption := 'Exit trade';
    GroupBoxOptionManagement.Caption := 'Save && Apply';
    GroupBox3.Caption := 'Loss cut';
    GroupBox1.Caption := 'Profit taking';
    GroupBox2.Caption := 'Trailing Stop';

    CheckBoxUSE_LOSSCUT.Caption := 'Use';
    Label1.Caption := '- X% of Entry Price';

    GroupBox1.Caption := 'Profit Taking';
    CheckBoxUSE_PROFITCUT.Caption := 'Use';
    Label3.Caption := '+ X% of Entry Price';

    CheckBoxUSE_TRAILINGSTOP.Caption := 'Use';
    Label5.Caption := 'Target Price : X% of Entry Price';
    Label7.Caption := 'Exit Price : Y% of Max Profit';
  end;
end;
...
// -----------------------------------------------------------------------------
// 외부에서 옵션객체를 설정한다.
procedure TExit_Frame.AssignOption(AOption: CMXTradeStrategyOption);
begin
