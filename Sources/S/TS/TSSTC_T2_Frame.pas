...
type
  TSTC_T2_Frame = class(TTradeStrategyFrame)
    Panel1: TPanel;
    Panel3: TPanel;
...
uses FNGlobal, FNCMVariable, MXTSVariable, TSSelPriceLineDlg,
  MKTradeStrategyConst;

procedure TSTC_T2_Frame.ApplyLanguage;
begin
...
constructor TSTC_T2_Frame.Create(AOwner: TComponent);
begin
...
procedure TSTC_T2_Frame.MakeDefaultOption(AOption: CMXTradeStrategyOption);
begin
  CMXTradeStrategyOption.Default_STC_T2(AOption);
end;

// 화면컨트롤에서 데이터를 가져온다.
procedure TSTC_T2_Frame.GetControlData;
begin
...
procedure TSTC_T2_Frame.SetControlData;
begin
...
procedure TSTC_T2_Frame.ButtonPRICEMETHODClick(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
...
procedure TSTC_T2_Frame.ButtonApplyTSOptionClick(Sender: TObject);
begin
  ApplyOptionCollection;
end;

procedure TSTC_T2_Frame.ButtonFavorClick(Sender: TObject);
begin
  AddFavorOptionCollection;
end;

procedure TSTC_T2_Frame.ButtonOptionManagementClick(Sender: TObject);
begin
  ManagementOptionCollection;
end;

procedure TSTC_T2_Frame.ButtonPopupMenuClick(Sender: TObject);
var
  f_PT: TPoint;
begin
...
procedure TSTC_T2_Frame.ComboBoxMajorTypeChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
...
procedure TSTC_T2_Frame.ComboBoxOptionCollectionChange(Sender: TObject);
begin
  ApplyOptionCollection;
end;

procedure TSTC_T2_Frame.EditKeyPress(Sender: TObject; var Key: Char);
begin
  if Key in [#$D] then
  begin
    OptionChange(Sender);
...
procedure TSTC_T2_Frame.UpdateControlData;
begin
  SetControlData;
end;

procedure TSTC_T2_Frame.UpDownClick(Sender: TObject; Button: TUDBtnType);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
...
procedure TSTC_T2_Frame.OptionChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
