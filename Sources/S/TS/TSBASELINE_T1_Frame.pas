...
type
  TBASELINE_T1_Frame = class(TTradeStrategyFrame)
    Panel1: TPanel;
    GroupBox2: TGroupBox;
    Panel2: TPanel;
    Panel3: TPanel;
...
constructor TBASELINE_T1_Frame.Create(AOwner: TComponent);
begin
...
procedure TBASELINE_T1_Frame.MakeDefaultOption(AOption: CMXTradeStrategyOption);
begin
  CMXTradeStrategyOption.Default_BASELINE_T1(AOption);
end;

// 화면컨트롤에서 데이터를 가져온다.
procedure TBASELINE_T1_Frame.GetControlData;
begin
...
procedure TBASELINE_T1_Frame.SetControlData;
begin
...
procedure TBASELINE_T1_Frame.ButtonPRICEMETHODClick(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
...
procedure TBASELINE_T1_Frame.ButtonApplyTSOptionClick(Sender: TObject);
begin
  ApplyOptionCollection;
end;

procedure TBASELINE_T1_Frame.ButtonFavorClick(Sender: TObject);
begin
  AddFavorOptionCollection;
end;

procedure TBASELINE_T1_Frame.ButtonOptionManagementClick(Sender: TObject);
begin
  ManagementOptionCollection;
end;

procedure TBASELINE_T1_Frame.ButtonPopupMenuClick(Sender: TObject);
var
  f_PT: TPoint;
begin
...
procedure TBASELINE_T1_Frame.ComboBoxMajorTypeChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
...
procedure TBASELINE_T1_Frame.ComboBoxOptionCollectionChange(Sender: TObject);
begin
  ApplyOptionCollection;
end;

procedure TBASELINE_T1_Frame.EditKeyPress(Sender: TObject; var Key: Char);
begin
  if Key in [#$D] then
  begin
    EditChange(Sender);
...
procedure TBASELINE_T1_Frame.UpdateControlData;
begin
  SetControlData;
end;

procedure TBASELINE_T1_Frame.UpDownClick(Sender: TObject; Button: TUDBtnType);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
...
procedure TBASELINE_T1_Frame.EditChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
...
procedure TBASELINE_T1_Frame.OptionChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
