...
type
  TIM_T2_Frame = class(TTradeStrategyFrame)
    Panel1: TPanel;
    Panel3: TPanel;
    ...
    Label5: TLabel;
    Label6: TLabel;
    RadioButtonM2: TRadioButton;
    RadioButtonM1: TRadioButton;
    RadioButtonM4: TRadioButton;
    EditM1V1: TEdit;
    EditM2V1: TEdit;
    EditM4V1: TEdit;
    RadioButtonM3: TRadioButton;
    EditM3V1: TEdit;
    CheckBoxUSE_SIDECONDITION: TCheckBox;
    ComboBoxVALUE_TYPE: TComboBox;
    GroupBox3: TGroupBox;
    Label7: TLabel;
...
constructor TIM_T2_Frame.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  m_ComboBoxOptionCollection := ComboBoxOptionCollection;
  CMXTradeStrategyOption.Default_IM_T2_O1(m_Option);
...
procedure TIM_T2_Frame.MakeDefaultOption(AOption: CMXTradeStrategyOption);
begin
  CMXTradeStrategyOption.Default_IM_T2_O1(AOption);
end;

// 화면컨트롤에서 데이터를 가져온다.
procedure TIM_T2_Frame.GetControlData;
begin
  ...
  if RadioButtonM1.Checked then
  begin
    m_Option.SetIntegerValue('TREND_FILTER', 0);
  end
  else if RadioButtonM2.Checked then
  begin
    m_Option.SetIntegerValue('TREND_FILTER', 1);
  end
  else if RadioButtonM3.Checked then
  begin
    m_Option.SetIntegerValue('TREND_FILTER', 2);
  end
  else
  begin
    m_Option.SetIntegerValue('TREND_FILTER', 3);
  end;

  m_Option.SetDoubleValue('M1V1', TFNGlobal.atof(EditM1V1.Text));
  m_Option.SetDoubleValue('M2V1', TFNGlobal.atof(EditM2V1.Text));
  m_Option.SetDoubleValue('M3V1', TFNGlobal.atoi(EditM3V1.Text));
  m_Option.SetDoubleValue('M4V1', TFNGlobal.atof(EditM4V1.Text));
  m_Option.SetIntegerValue('VALUE_TYPE', ComboBoxVALUE_TYPE.ItemIndex);
  m_Option.SetBooleanValue('USE_SIDECONDITION', CheckBoxUSE_SIDECONDITION.Checked);
...
procedure TIM_T2_Frame.SetControlData;
var
  f_NValue: Integer;
begin
  ...
  try
    ...
    f_NValue := m_Option.GetIntegerValue('TREND_FILTER');

    if 0 = f_NValue then
    begin
      RadioButtonM1.Checked := true;
    end
    else if 1 = f_NValue then
    begin
      RadioButtonM2.Checked := true;
    end
    else if 2 = f_NValue then
    begin
      RadioButtonM3.Checked := true;
    end
    else
    begin
      RadioButtonM4.Checked := true;
    end;

    EditM1V1.Text := FloatToStr(m_Option.GetDoubleValue('M1V1'));
    EditM2V1.Text := FloatToStr(m_Option.GetDoubleValue('M2V1'));
    EditM3V1.Text := FloatToStr(m_Option.GetIntegerValue('M3V1'));
    EditM4V1.Text := FloatToStr(m_Option.GetDoubleValue('M4V1'));
    ComboBoxVALUE_TYPE.ItemIndex := m_Option.GetIntegerValue('VALUE_TYPE');
    CheckBoxUSE_SIDECONDITION.Checked := m_Option.GetBooleanValue('USE_SIDECONDITION');
...
procedure TIM_T2_Frame.ButtonApplyTSOptionClick(Sender: TObject);
begin
  ApplyOptionCollection;
end;

procedure TIM_T2_Frame.ButtonFavorClick(Sender: TObject);
begin
  AddFavorOptionCollection;
end;

procedure TIM_T2_Frame.ButtonOptionManagementClick(Sender: TObject);
begin
  ManagementOptionCollection;
end;

procedure TIM_T2_Frame.ButtonPopupMenuClick(Sender: TObject);
var
  f_PT: TPoint;
begin
...
procedure TIM_T2_Frame.ComboBoxMajorTypeChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
...
procedure TIM_T2_Frame.ComboBoxOptionCollectionChange(Sender: TObject);
begin
  ApplyOptionCollection;
end;

procedure TIM_T2_Frame.EditKeyPress(Sender: TObject; var Key: Char);
begin
  if Key in [#$D] then
  begin
    EditChange(Sender);
...
procedure TIM_T2_Frame.UpdateControlData;
begin
  SetControlData;
end;

procedure TIM_T2_Frame.UpDownClick(Sender: TObject; Button: TUDBtnType);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
...
procedure TIM_T2_Frame.EditChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
...
procedure TIM_T2_Frame.OptionChange(Sender: TObject);
begin
  if not Assigned(m_Option) then
    exit;
  if not m_EnableEvent then
