unit FNQStreamChartDataSeries;

interface
uses
    SysUtils, Math, DateUtils, FNQChartData, FNQChartDataSeries, FNQuotData, FNDataSet, FNPOTCollection;

type
    CFNQStreamChartDataSeries = class(CFNQChartDataSeries)
    private
...
//---------------------------------------------------------------------------
constructor CFNQStreamChartDataSeries.Create();
begin
    inherited Create();
...
//---------------------------------------------------------------------------
destructor CFNQStreamChartDataSeries.Destroy();
begin
    m_POTItem.Free;
    m_QuotData.Free;
...
//당일의 거래시간 정보를 설정한다.
//---------------------------------------------------------------------------
procedure CFNQStreamChartDataSeries.SetPOTItem(p_POTItem: CFNPOTItem);
begin
    if Assigned(p_POTItem) then m_POTItem.Clone(p_POTItem);
end;

//---------------------------------------------------------------------------
function CFNQStreamChartDataSeries.UpdateQuotData(p_QuotData:CFNQuotData): Integer;
var
    f_AddCount  : Integer;
begin
...
function CFNQStreamChartDataSeries.UpdateQuotDataToMin(p_QuotData: CFNQuotData): Integer;
var
    f_Time                  : Double;
    f_Index                 : Integer;
    m_OldMinIndexOnToday    : Integer;
    f_MarketData            : CFNQChartData;
    f_MarketData1           : CFNQChartData;
    f_AddCount              : Integer;
...
begin
    ...
    begin
        f_MarketData            := CFNQChartData(m_Items.Items[m_OldMinIndex]);
        ...
        f_MarketData.m_CloseDateTime :=
            EncodeDateTime(f_MarketData.m_Year, f_MarketData.m_Month, f_MarketData.m_Day, f_MarketData.m_Hour, f_MarketData.m_Min, f_MarketData.m_Sec, 0);
    end;
    ...
    begin
        ...
        begin
            f_MarketData            := CFNQChartData.Create();
            f_MarketData.m_Year     := p_QuotData.m_Year;
            f_MarketData.m_Month    := p_QuotData.m_Month;
            f_MarketData.m_Day      := p_QuotData.m_Day;

            f_Time                  := IndexToOpenTime(f_Index-m_ZeroIndex);
            f_MarketData.m_OpenDateTime :=
                EncodeDateTime(p_QuotData.m_Year, p_QuotData.m_Month, p_QuotData.m_Day, CFNPOTItem.NumberToHour(f_Time), CFNPOTItem.NumberToMin(f_Time), CFNPOTItem.NumberToSec(f_Time), 0);
            ...
            begin
                f_MarketData.m_OpenOPS   := CFNQChartData(m_Items.Items[f_Index-1]).m_CloseOPS;
                ...
                f_MarketData.m_OpenPrice    := CFNQChartData(m_Items.Items[f_Index-1]).m_ClosePrice;
        ...
        if f_MarketData.m_HighOPS < f_MarketData.m_OpenOPS then f_MarketData.m_HighOPS := f_MarketData.m_OpenOPS;
        if f_MarketData.m_LowOPS  > f_MarketData.m_OpenOPS then f_MarketData.m_LowOPS  := f_MarketData.m_OpenOPS;
        ...
        if f_MarketData.m_HighPrice < f_MarketData.m_OpenPrice then f_MarketData.m_HighPrice := f_MarketData.m_OpenPrice;
        if f_MarketData.m_LowPrice  > f_MarketData.m_OpenPrice then f_MarketData.m_LowPrice  := f_MarketData.m_OpenPrice;
    ...
    begin
        ...
        f_MarketData         := CFNQChartData(m_Items.Items[m_NewMinIndex]);
        f_MarketData.m_Year  := p_QuotData.m_Year;
        f_MarketData.m_Month := p_QuotData.m_Month;
        f_MarketData.m_Day   := p_QuotData.m_Day;
        f_MarketData.m_Hour  := p_QuotData.m_Hour;
        ...
        if (f_MarketData.m_HighOPS < p_QuotData.m_CloseOPS) then f_MarketData.m_HighOPS := p_QuotData.m_CloseOPS;
        if (f_MarketData.m_LowOPS  > p_QuotData.m_CloseOPS) then f_MarketData.m_LowOPS  := p_QuotData.m_CloseOPS;

        f_MarketData.m_CloseOPS     := p_QuotData.m_CloseOPS;

        if (f_MarketData.m_HighPrice < p_QuotData.m_ClosePrice) then f_MarketData.m_HighPrice := p_QuotData.m_ClosePrice;
        if (f_MarketData.m_LowPrice  > p_QuotData.m_ClosePrice) then f_MarketData.m_LowPrice  := p_QuotData.m_ClosePrice;
...
function CFNQStreamChartDataSeries.UpdateQuotDataToDay(p_QuotData: CFNQuotData): Integer;
var
    f_Index     : Integer;
    f_FindIndex : Integer;
    f_MarketData: CFNQChartData;
...
//---------------------------------------------------------------------------
function CFNQStreamChartDataSeries.UpdateStreamQuotData(p_StreamRecord:CFNStreamRecord): Integer;
begin
...
//---------------------------------------------------------------------------
//당일의 시작위치를 찾는다.
function CFNQStreamChartDataSeries.FindZeroIndex(p_Date: TDateTime): Integer;
var
    f_MarketData    : CFNQChartData;
    f_Index         : Integer;
    f_ZeroIndex     : Integer;
...
begin
    ...
    while (f_Index >= 0) do
    begin
        f_MarketData := CFNQChartData(m_Items.Items[f_Index]);
    ...
    begin
        ...
        begin
            f_MarketData    := CFNQChartData(m_Items.Items[0]);
    ...
    begin
        f_MarketData := CFNQChartData(m_Items.Items[m_Items.Count-1]);
...
procedure CFNQStreamChartDataSeries.FindZeroNextTime;
begin
...
procedure CFNQStreamChartDataSeries.ReadyStream;
var
...
function CFNQStreamChartDataSeries.CloseTimeToIndex(p_Hour, p_Min, p_Sec: Integer): Integer;
var
    f_HourIndex     : Integer;
    f_Index         : Integer;
    f_MaxIndex      : Integer;
...
//---------------------------------------------------------------------------
//해당 인덱스의 마감 시간을 계산한다.
function CFNQStreamChartDataSeries.IndexToCloseTime(p_Index: Integer): Double;
var
    f_OpenTime      : Double;
    f_FindTime      : Double;
    f_Zero          : Integer;
...
//---------------------------------------------------------------------------
function CFNQStreamChartDataSeries.GetOpenDateTime(p_CloseDateTime:Double; p_Factor:Integer) : Double;
var
...
//---------------------------------------------------------------------------
//해당 인덱스의 시작 시간을 계산한다.
function CFNQStreamChartDataSeries.IndexToOpenTime(p_Index: Integer): Double;
var
    f_OpenTime      : Double;
    f_FindTime      : Double;
    f_Zero          : Integer;
