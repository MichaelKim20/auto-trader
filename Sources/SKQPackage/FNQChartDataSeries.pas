unit FNQChartDataSeries;

interface

uses
    SysUtils, Classes, Math, DateUtils, ExtCtrls, FNQChartData;

type
    CFNQChartDataSeries = class(TObject)
    public
        m_Items     : TList;
        m_TimeFrame : Integer;
        m_Precision : Integer;
    ...
    public
        ...
        procedure Add(p_MarketData:CFNQChartData);
        procedure AddTick(p_MarketData:CFNQChartData);
        procedure Clone(p_Source:CFNQChartDataSeries);
...
//------------------------------------------------------------------------------
constructor CFNQChartDataSeries.Create();
begin
    inherited Create;
...
//------------------------------------------------------------------------------
destructor CFNQChartDataSeries.Destroy();
begin
    Clear();
...
//------------------------------------------------------------------------------
procedure CFNQChartDataSeries.Clear();
begin
    while 0 < m_Items.Count  do
    begin
        CFNQChartData(m_Items.Items[0]).Free();
        m_Items.Delete(0);
    end;
end;

//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchDayByClose(p_TimeDate: TDateTime; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
begin
    ...
    begin
        ...
        repeat
            ...
            f_MarketData    := CFNQChartData(m_Items.Items[f_PosX]);
...
//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchWeekByClose(p_TimeDate: TDateTime; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchMonthByClose(p_TimeDate: TDateTime; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchByClose(p_TimeDate: TDateTime; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchByClose2(p_TimeDate: TDateTime; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchByClose3(p_TimeDate: TDateTime; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchByKey(p_Key: String; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
//------------------------------------------------------------------------------
function CFNQChartDataSeries.SearchByOpen(p_TimeDate: TDateTime; p_Nearest: Boolean): Integer;
var
    f_PosX          : Integer;
    f_PosL          : Integer;
    f_PosR          : Integer;
...
//------------------------------------------------------------------------------
procedure CFNQChartDataSeries.Add(p_MarketData: CFNQChartData);
var
...
begin
    ...
    begin
        ...
        if (p_MarketData.m_CloseDateTime < CFNQChartData(m_Items.Items[0]).m_CloseDateTime) then
        begin
            m_Items.Insert(0, p_MarketData);
        end else
        begin
            //만약 추가할 데이터의 날짜가 가장 앞쪽의 데이터 보다 크다면 가장뒤에 넣는다.
            if (p_MarketData.m_CloseDateTime > CFNQChartData(m_Items.Items[m_Items.Count - 1]).m_CloseDateTime) then
            begin
            ...
            begin
                ...
                begin
                    if (p_MarketData.m_CloseDateTime = CFNQChartData(m_Items.Items[f_SearchIndex]).m_CloseDateTime) then
                    begin
                        CFNQChartData(m_Items.Items[f_SearchIndex]).Free();
...
//------------------------------------------------------------------------------
procedure CFNQChartDataSeries.AddTick(p_MarketData: CFNQChartData);
var
    f_SearchIndex:Integer;
begin
    if (0 < m_Items.Count) then
    begin
        if (p_MarketData.m_Key < CFNQChartData(m_Items.Items[0]).m_Key) then
        begin
            m_Items.Insert(0, p_MarketData);
        end else
        if (p_MarketData.m_Key >= CFNQChartData(m_Items.Items[m_Items.Count - 1]).m_Key) then
        begin
            m_Items.Add(p_MarketData);
        end else
        begin
...
//------------------------------------------------------------------------------
procedure CFNQChartDataSeries.RemoveVirtualData;
var
    f_OldMarketData : CFNQChartData;
    f_NewMarketData : CFNQChartData;
    f_Index         : Integer;
    m_TempList      : TList;
begin
         (*
    ...
    begin
        ...
        begin
            f_NewMarketData := CFNQChartData.Create();
...
//------------------------------------------------------------------------------
procedure CFNQChartDataSeries.AddVirtualDataAtOpening(ACount:Integer);
var
    f_MarketData0 : CFNQChartData;
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    f_NewMarketData.m_OpenDateTime  := f_MarketData0.m_OpenDateTime;
                    f_NewMarketData.m_CloseDateTime := f_MarketData0.m_OpenDateTime;
...
//------------------------------------------------------------------------------
procedure CFNQChartDataSeries.Clone(p_Source: CFNQChartDataSeries);
var
    f_OldMarketData : CFNQChartData;
    f_NewMarketData : CFNQChartData;
    f_Index         : Integer;
begin
    ...
    begin
        ...
        begin
            f_OldMarketData := CFNQChartData(p_Source.m_Items.Items[f_Index]);
            f_NewMarketData := CFNQChartData.Create();
            f_NewMarketData.Clone(f_OldMarketData);
            m_Items.Add(f_NewMarketData);
        end;
...
//------------------------------------------------------------------------------
procedure CFNQChartDataSeries.SaveToFile(p_FileName: String);
var
    f_Index         : Integer;
    f_Mode:Word;
    ...
    try
        ...
        begin
            ...
            begin
                ...
                f_Line :=
                    Format('%d', [f_RecordIndex]) + ',' +
                    DateToStr(f_ChartData.m_CloseDateTime) + ',' +
                    TFNGlobal.DateTimeToStr6(f_ChartData.m_CloseDateTime) + ',' +
                    Format('%.6f', [f_ChartData.m_CloseOPS]) + ',' +
