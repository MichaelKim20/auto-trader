...
uses
  SysUtils, Classes, Dialogs, MKIndicatorValue, FNRegistry;

type

  CMKSetting = class(TObject)
  public const
    IND_BB: Integer = 0;
...
uses
  MKGlobal, MKConst, MKChartDefine;

// ---------------------------------------------------------------------------
constructor CMKSetting.Create();
begin
  inherited Create();
...
// ---------------------------------------------------------------------------
procedure CMKSetting.Initialize;
var
  f_Index: Integer;
  f_Indicator: CMKIndicatorValue;
begin
  m_SelectedIndexO := -1;
  m_SelectedIndexI := -1;
  m_SelectedIndexS := -1;
  ...
  m_Scale := 0;
  m_TraceVisible := true;
  m_ColorSetIndex := CMKConst.COLOR_SET_WHITE;
  ...
  /// /////////////////////////////////////////////
  f_Indicator := CMKIndicatorValue.Create();
  ...
  f_Indicator.m_OptionLabel[1] := '표준편차 배수';

  f_Indicator.m_OptionMaximum[0] := 1200;
  ...
  f_Indicator.m_OptionMaximum[1] := 3;
  ...
  f_Indicator.m_OptionMaximum[0] := 1200;
  f_Indicator.m_OptionMaximum[1] := 1200;
  f_Indicator.m_OptionMaximum[2] := 1200;
  f_Indicator.m_OptionMinimum[0] := 1;
  f_Indicator.m_OptionMinimum[1] := 1;
  ...
  f_Indicator.m_OptionMaximum[1] := 1200;
  f_Indicator.m_OptionMaximum[2] := 1200;
  f_Indicator.m_OptionMaximum[3] := 1200;
  f_Indicator.m_OptionMinimum[0] := 1;
  f_Indicator.m_OptionMinimum[1] := 1;
  f_Indicator.m_OptionMinimum[2] := 1;
  ...
  begin
    f_Indicator := CMKIndicatorValue(m_Indicator[f_Index]);
    f_Indicator.SetDefaultValue();
  end;
end;

// ---------------------------------------------------------------------------
procedure CMKSetting.Finalize();
var
  f_Index: Integer;
begin
  while (0 < m_Indicator.Count) do
  begin
    CMKIndicatorValue(m_Indicator[0]).Free();
...
// ---------------------------------------------------------------------------
procedure CMKSetting.AddI(p_Value: Integer);
var
  f_FindIndex: Integer;
  f_newValue: ^Integer;
begin
...
// ---------------------------------------------------------------------------
// 오버레이지표를 추가한다.
procedure CMKSetting.AddO(p_Value: Integer);
var
  f_FindIndex: Integer;
  f_newValue: ^Integer;
begin
...
// ---------------------------------------------------------------------------
procedure CMKSetting.AddS(p_Value: Integer);
var
  f_FindIndex: Integer;
  f_newValue: ^Integer;
begin
...
function CMKSetting.DeleteI(p_Value: Integer): Integer;
var
  f_FindIndex: Integer;
begin
  f_FindIndex := FindI(p_Value);
...
// ---------------------------------------------------------------------------
function CMKSetting.DeleteO(p_Value: Integer): Integer;
var
  f_FindIndex: Integer;
begin
  f_FindIndex := FindO(p_Value);
...
// ---------------------------------------------------------------------------
function CMKSetting.DeleteS(p_Value: Integer): Integer;
var
  f_FindIndex: Integer;
begin
  f_FindIndex := FindS(p_Value);
...
// ---------------------------------------------------------------------------
function CMKSetting.FindI(p_Value: Integer): Integer;
var
  f_Index: Integer;
  f_Find: Integer;
begin
...
// ---------------------------------------------------------------------------
function CMKSetting.FindIdentity(p_Value: String): Integer;
var
  f_Index: Integer;
  f_Find: Integer;
begin
  ...
  for f_Index := 0 to m_Indicator.Count - 1 do
  begin
    if (CMKIndicatorValue(m_Indicator[f_Index]).m_Name = p_Value) then
    begin
      f_Find := f_Index;
      break;
    end;
...
// ---------------------------------------------------------------------------
function CMKSetting.FindO(p_Value: Integer): Integer;
var
  f_Index: Integer;
  f_Find: Integer;
begin
...
// ---------------------------------------------------------------------------
function CMKSetting.FindS(p_Value: Integer): Integer;
var
  f_Index: Integer;
  f_Find: Integer;
begin
...
// ---------------------------------------------------------------------------
function CMKSetting.GetFirstValueI: Integer;
var
  f_Index: Integer;
  f_First: Integer;
begin
...
// ---------------------------------------------------------------------------
function CMKSetting.GetFirstValueO: Integer;
var
  f_Index: Integer;
  f_First: Integer;
begin
...
// ---------------------------------------------------------------------------
function CMKSetting.GetFirstValueS: Integer;
var
  f_Index: Integer;
  f_First: Integer;
begin
...
// ---------------------------------------------------------------------------
function CMKSetting.GetIndCountI: Integer;
begin
  Result := m_SequenceI.Count;
end;

// ---------------------------------------------------------------------------
function CMKSetting.GetIndCountO: Integer;
begin
  Result := m_SequenceO.Count;
end;

// ---------------------------------------------------------------------------
function CMKSetting.GetIndCountS: Integer;
begin
  Result := m_SequenceS.Count;
end;

// ---------------------------------------------------------------------------
procedure CMKSetting.SetRegistry(f_Registry: CFNRegistry);
begin
  m_Registry := f_Registry;
end;

// ---------------------------------------------------------------------------
procedure CMKSetting.SetRegSection(f_Section: String);
begin
  m_RegSection := f_Section;
end;

// ---------------------------------------------------------------------------
procedure CMKSetting.SaveSetting();
var
  f_Sequence: String;
  f_Index: Integer;
...
begin
  ...
  begin
    try
      ...
      begin
        f_Sequence := '';
        for f_Option := 0 to CMKIndicatorValue(m_Indicator.Items[f_Index]).m_OptionCount - 1 do
        begin
          f_Sequence := f_Sequence + FloatToStr(CMKIndicatorValue(m_Indicator.Items[f_Index]).m_OptionValue[f_Option]);
          if (f_Option < CMKIndicatorValue(m_Indicator.Items[f_Index]).m_OptionCount - 1) then
            f_Sequence := f_Sequence + ',';
        end;
        if f_Sequence = '' then
          f_Sequence := ' ';
...
// ---------------------------------------------------------------------------
procedure CMKSetting.ReadSetting();
var
  f_Buffer: String;
...
begin
  ...
  begin
    try
      ...
      if m_TimeFrameIndex < 0 then
        m_TimeFrameIndex := 0;
      if m_TimeFrameIndex >= MAXTIMEFRAMECOUNT then
        m_TimeFrameIndex := 0;
      ...
      begin
        ...
        begin
          ...
          begin
            AddO(TMKGlobal.atoi(f_StringList[f_Index]));
          end;
        end;
        m_SelectedIndexO := TMKGlobal.atoi(m_Registry.ReadString(m_RegSection, 'SelectedIndexO', '0'));
      ...
      // AddO(IND_MA);
      if (GetIndCountO() >= (CMKConst.MAX_OVERLAY_COUNT)) then
      begin
      ...
      begin
        ...
        begin
          if (0 < Length(f_StringList[f_Index])) then
          begin
            AddI(TMKGlobal.atoi(f_StringList[f_Index]));
          end;
        end;
        m_SelectedIndexI := TMKGlobal.atoi(m_Registry.ReadString(m_RegSection, 'SelectedIndexI', '0'));
      ...
      if (GetIndCountI() >= (CMKConst.MAX_INDICATOR_COUNT)) then
      begin
      ...
      begin
        ...
        begin
          if (0 < Length(f_StringList[f_Index])) then
          begin
            AddS(TMKGlobal.atoi(f_StringList[f_Index]));
          end;
        end;
        m_SelectedIndexS := TMKGlobal.atoi(m_Registry.ReadString(m_RegSection, 'SelectedIndexS', '0'));
      ...
      if (GetIndCountS() >= (CMKConst.MAX_SIGNAL_COUNT)) then
      begin
      ...
      begin
        ...
        CMKIndicatorValue(m_Indicator.Items[f_Identity]).m_ViewIndicator := true;
      end;
          ...
          begin
            ...
            begin
              if (0 < Length(f_OptionsList[f_Option])) then
                CMKIndicatorValue(m_Indicator.Items[f_Index]).m_OptionValue[f_Option] := StrToFloat(f_OptionsList[f_Option])
              else
                CMKIndicatorValue(m_Indicator.Items[f_Index]).m_OptionValue[f_Option] :=
                    CMKIndicatorValue(m_Indicator.Items[f_Index]).m_OptionDefaultValue[f_Option];
            end;
          end;
        end;
      end;
...
// ---------------------------------------------------------------------------
procedure CMKSetting.ClearSequenceO();
begin
  while (0 < m_SequenceO.Count) do
  begin
    Dispose(m_SequenceO.Items[0]);
...
// ---------------------------------------------------------------------------
procedure CMKSetting.ClearSequenceI();
begin
  while (0 < m_SequenceI.Count) do
  begin
    Dispose(m_SequenceI.Items[0]);
...
// ---------------------------------------------------------------------------
procedure CMKSetting.ClearSequenceS();
begin
  while (0 < m_SequenceS.Count) do
  begin
    Dispose(m_SequenceS.Items[0]);
...
// ---------------------------------------------------------------------------
procedure CMKSetting.DeleteAll;
var
  f_Index: Integer;
begin
  ClearSequenceO();
  ...
  for f_Index := 0 to m_Indicator.Count - 1 do
  begin
    CMKIndicatorValue(m_Indicator.Items[f_Index]).m_ViewIndicator := false;
  end;
end;
