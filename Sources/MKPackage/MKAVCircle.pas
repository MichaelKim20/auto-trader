...
type
  /// /////////////////////////////////////////////////////////////////////////
  //
  CMKAVCircle = class(CMKAVDrawingObject)
  private
    m_X1Date: TDateTime;
    m_X2Date: TDateTime;
    m_X1: Double;
  ...
  public
    ...
    procedure DrawArc(p_Bitmap: TBitmap32; cx: Integer; cy: Integer; w: Integer; h: Integer; s: Integer; e: Integer);
    procedure ClearText();
  end;
...
// CMKAVVLine
// ---------------------------------------------------------------------------
constructor CMKAVCircle.Create();
begin
  inherited Create;
...
// ---------------------------------------------------------------------------
destructor CMKAVCircle.Destroy();
begin
  ClearText();
...
// ---------------------------------------------------------------------------
procedure CMKAVCircle.ClearText;
begin

end;

// ---------------------------------------------------------------------------
procedure CMKAVCircle.XValueToDate;
begin
  if (m_X1 >= 0) then
...
// ---------------------------------------------------------------------------
procedure CMKAVCircle.XDateToValue;
begin
  m_X1 := GetIndex(m_X1Date);
  m_X2 := GetIndex(m_X2Date);
end;

procedure CMKAVCircle.DrawArc(p_Bitmap: TBitmap32; cx: Integer; cy: Integer; w: Integer; h: Integer; s: Integer; e: Integer);
var
  f_X_pos: Integer;
  f_Y_pos: Integer;
  f_Index4: Integer;
...
begin
  ...
  f_Pi := pi; // 3.1415926535;
  f_X_pos := Math.Floor((Cos(f_Pi * s / 180) * w / 2) + cx);
  f_Y_pos := Math.Floor((Sin(f_Pi * s / 180) * h / 2) + cy);

  p_Bitmap.MoveTo(f_X_pos, f_Y_pos);
  for f_Index4 := s to e do
  begin
    f_X_pos := Math.Floor((Cos(f_Pi * f_Index4 / 180) * w / 2) + cx);
    f_Y_pos := Math.Floor((Sin(f_Pi * f_Index4 / 180) * h / 2) + cy);
    p_Bitmap.LineToAS(f_X_pos, f_Y_pos);
  end;
end;

// ---------------------------------------------------------------------------
function CMKAVCircle.HitTest(p_X, p_Y: Integer): Boolean;
var
  f_LXMin: Double;
  f_LXMax: Double;
  f_LYMin: Double;
...
begin
  ...
  begin
    f_XR := Abs(f_X1 - f_X2) / 2;
    f_YR := Abs(f_Y1 - f_Y2) / 2;
    f_TX := p_X - GetScreenXCenter(m_cX);
    if (0 < (f_XR * f_XR - f_TX * f_TX)) then
    begin
      f_TY := Sqrt(f_XR * f_XR - f_TX * f_TX) * f_YR / f_XR;
    end
    else
    begin
      f_TY := 0;
    ...
    if (GetScreenY(m_cY) > p_Y) then
      f_TY := f_TY * -1;

    f_TY := f_TY + GetScreenY(m_cY);
    ...
    else if ((p_Y - m_SelectMaxMin <= f_TY) and (f_TY <= p_Y + m_SelectMaxMin)) then
    begin
      m_SelectType := 0;
      m_Hit := true;
    end;
...
// ---------------------------------------------------------------------------
procedure CMKAVCircle.OnMouseDown(p_X, p_Y: Integer);
begin
  if (m_Drawing) then
  begin
    m_X1 := GetRealX(p_X);
...
// ---------------------------------------------------------------------------
procedure CMKAVCircle.OnMouseUp(p_X, p_Y: Integer);
begin
  if (m_Drawing) then
  begin
    m_X2 := GetRealX(p_X);
...
// ---------------------------------------------------------------------------
procedure CMKAVCircle.OnMouseMove(p_X, p_Y: Integer);
begin
...
// ---------------------------------------------------------------------------
procedure CMKAVCircle.DrawDefaultLayer(p_Bitmap: TBitmap32);
var
  f_X1, f_X2: Integer;
  f_Y1, f_Y2: Integer;
  f_Label: String;
...
begin
  ...
  m_cX := Math.Floor((f_LXMin + f_LXMax) / 2);
  m_cY := (f_LYMax + f_LYMin) / 2;
  f_Width := Abs(f_X1 - f_X2);
  f_Height := Abs(f_Y1 - f_Y2);
  f_CX := GetScreenXCenter(m_cX);
  f_CY := Math.Floor(GetScreenY(m_cY));
  ...
  begin
    ...
    DrawArc(p_Bitmap, f_CX, f_CY, f_Width, f_Height, m_StartAngle, m_EndAngle);
