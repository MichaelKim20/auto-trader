unit MXTradeStrategyDISPARITY_T1;
...
type
  // ---------------------------------------------------------------------------
  // 이격도 추세 1에 사용하는 보조지표와 신호를 계산하는 클래스
  // ---------------------------------------------------------------------------
  CMXTradeStrategyDISPARITY_T1 = class(CMXTradeStrategy)
  public
    // 생성자
    constructor Create();
...
// ---------------------------------------------------------------------------
// 생성자
constructor CMXTradeStrategyDISPARITY_T1.Create();
begin
  inherited Create();

  CMXTradeStrategyOption.Default_DISPARITY_T1(m_Option);
...
// ---------------------------------------------------------------------------
// 파괴자
destructor CMXTradeStrategyDISPARITY_T1.Destroy();
begin
  inherited Destroy();
end;
...
// ---------------------------------------------------------------------------
// 계산에 필요한 라인을 생성한다.
procedure CMXTradeStrategyDISPARITY_T1.CreateLineValueSeries;
var
...
begin
  ...
  f_LineValueSeries.m_Name := 'ST-DISPARITY-T1-L1';
  ...
  f_LineValueSeries.m_Name := 'ST-DISPARITY-T1-S1';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
...
// ---------------------------------------------------------------------------
// 계산을 시작한다.
procedure CMXTradeStrategyDISPARITY_T1.Calculate(ARecalculation: Boolean);
var
...
// ---------------------------------------------------------------------------
// 신호를 계산한다.
procedure CMXTradeStrategyDISPARITY_T1.CalulateSignal(p_Begin: Integer; p_End: Integer);
var
  f_Index: Integer;
...
{$REGION '계산에 필요한 변수 초기화'}
  f_Length2 := m_Option.GetIntegerValue('LENGTH2');
{$ENDREGION}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (f_SourceLineValue0.m_Value[2] > 100.0) and
              (f_LineValueSeries0.HighestPrice(f_Length2, f_LineValueSeries0, 2, f_Index - 1) < f_SourceLineValue0.m_Value[2])
          then
          begin
            f_SigLineValue0.m_Value[0] := STRATEGY_EXIT;

            if (f_PriceLineValue0.m_Value[3] - f_PriceLineValue0.m_Value[0]) > 0 then
            begin
        ...
        begin
          if (f_SourceLineValue0.m_Value[2] < 100.0) and
              (f_LineValueSeries0.LowestPrice(f_Length2, f_LineValueSeries0, 2, f_Index - 1) > f_SourceLineValue0.m_Value[2])
          then
          begin
            f_SigLineValue0.m_Value[0] := STRATEGY_EXIT;
