...
type
  // 일목 추세 1에 사용하는 보조지표와 신호를 계산하는 클래스
  CMXTradeStrategyIM_T1 = class(CMXTradeStrategy)
  public
    // 생성자
    constructor Create();
...
uses Math, MKGlobal, MKConst, MXTradeStrategyOptionCollection, FNMaterialCollection, FNCMVariable, MKTradeStrategyConst;

// ---------------------------------------------------------------------------
// 생성자
constructor CMXTradeStrategyIM_T1.Create();
begin
  inherited Create();
...
// ---------------------------------------------------------------------------
// 파괴자
destructor CMXTradeStrategyIM_T1.Destroy();
begin
  inherited Destroy();
end;
...
// ---------------------------------------------------------------------------
// 계산에 필요한 라인을 생성한다.
procedure CMXTradeStrategyIM_T1.CreateLineValueSeries;
var
...
begin
  ...
  f_LineValueSeries.m_Name := 'ST-IM-T1-L1';
  ...
  f_LineValueSeries.m_Name := 'ST-IM-T1-L2';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
  ...
  f_LineValueSeries.m_Name := 'ST-IM-T1-S1';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
...
// ---------------------------------------------------------------------------
// 옵션이 변경되어 재계산이 될 때 화면에 표시한 것들중에 옵션에 관련된 사항들을 적용한다.
procedure CMXTradeStrategyIM_T1.UpdateVisibleStateOfLineValueSeries;
var
  f_LineValueSeries: CMXTSLineValueSeries;
  f_Index: Integer;
begin
...
// ---------------------------------------------------------------------------
// 계산을 시작한다.
procedure CMXTradeStrategyIM_T1.Calculate(ARecalculation: Boolean);
var
...
// ---------------------------------------------------------------------------
// 신호를 계산한다.
procedure CMXTradeStrategyIM_T1.CalulateSignal(p_Begin: Integer; p_End: Integer);
var
  f_Index: Integer;
...
{$REGION '보조지표들의 특정을 비교하여 신호 계산'}
  ...
  begin
    ...
    begin
      ...
      if ((f_SourceLineValue10.m_Value[0] <> NOT_VALUE) AND (f_SourceLineValue00.m_Value[0] <> NOT_VALUE) AND
          (f_SourceLineValue00.m_Value[1] <> NOT_VALUE) AND (f_SourceLineValue00.m_Value[2] <> NOT_VALUE) AND
          (f_SourceLineValue00.m_Value[3] <> NOT_VALUE) AND (f_SourceLineValue10.m_Value[f_H_1_IDX] <> NOT_VALUE) AND
          (f_SourceLineValue10.m_Value[f_L_1_IDX] <> NOT_VALUE) AND (f_SourceLineValue10.m_Value[f_H_2_IDX] <> NOT_VALUE) AND
          (f_SourceLineValue10.m_Value[f_L_2_IDX] <> NOT_VALUE) AND (f_SourceLineValue10.m_Value[f_C_1_IDX] <> NOT_VALUE)) then
      begin
        f_Enable := false;
        ...
        begin
          ...
          begin
            if abs(f_SourceLineValue10.m_Value[f_H_1_IDX] - f_SourceLineValue10.m_Value[f_L_1_IDX]) <
                abs(f_SourceLineValue10.m_Value[f_H_2_IDX] - f_SourceLineValue10.m_Value[f_L_2_IDX]) *
...
{$REGION '전환선 > 기준선 => 매수'}
            ...
            if (f_SourceLineValue00.m_Value[0] > f_TargetValue0) then
            begin
