...
type
  // ---------------------------------------------------------------------------
  // 스토캐스틱 비추세 2에 사용하는 보조지표와 신호를 계산하는 클래스
  // ---------------------------------------------------------------------------
  CMXTradeStrategySTC_N2 = class(CMXTradeStrategy)
  public
    // 생성자
    constructor Create();
...
// ---------------------------------------------------------------------------
// 생성자
constructor CMXTradeStrategySTC_N2.Create();
begin
  inherited Create();
...
// ---------------------------------------------------------------------------
// 파괴자
destructor CMXTradeStrategySTC_N2.Destroy();
begin
  inherited Destroy();
end;
...
// ---------------------------------------------------------------------------
// 계산에 필요한 라인을 생성한다.
procedure CMXTradeStrategySTC_N2.CreateLineValueSeries;
var
...
begin
  ...
  f_LineValueSeries.m_Name := 'ST-STC-N2-L1';
  ...
  f_LineValueSeries.m_Name := 'ST-STC-N2-S1';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
...
// ---------------------------------------------------------------------------
// 옵션이 변경되어 재계산이 될 때 화면에 표시한 것들중에 옵션에 관련된 사항들을 적용한다.
procedure CMXTradeStrategySTC_N2.UpdateVisibleStateOfLineValueSeries;
var
  f_LineValueSeries: CMXTSLineValueSeries;
begin
...
// ---------------------------------------------------------------------------
// 계산을 시작한다.
procedure CMXTradeStrategySTC_N2.Calculate(ARecalculation: Boolean);
var
...
// ---------------------------------------------------------------------------
// 신호를 계산한다.
procedure CMXTradeStrategySTC_N2.CalulateSignal(p_Begin: Integer; p_End: Integer);
var
  f_Index: Integer;
...
{$REGION '보조지표들의 특정을 비교하여 신호 계산'}
  ...
  begin
    ...
    begin
      ...
      if ((f_SourceLineValue1.m_Value[2] <> NOT_VALUE) AND (f_SourceLineValue0.m_Value[2] <> NOT_VALUE)) then
      begin
        ...
        begin
          ...
          // %D가 20을 상향돌파 => 매수
          if (f_TargetValue >= f_SourceLineValue1.m_Value[2]) and (f_TargetValue < f_SourceLineValue0.m_Value[2]) then
          begin
