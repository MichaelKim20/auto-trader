...
type
  // ---------------------------------------------------------------------------
  // 이동평균선 비추세 1에 사용하는 보조지표와 신호를 계산하는 클래스
  // ---------------------------------------------------------------------------
  CMXTradeStrategyMOV_N1 = class(CMXTradeStrategy)
  public
    // 생성자
    constructor Create();
...
// ---------------------------------------------------------------------------
// 생성자
constructor CMXTradeStrategyMOV_N1.Create();
begin
  inherited Create();

  CMXTradeStrategyOption.Default_MOV_N1_O1(m_Option);
...
// ---------------------------------------------------------------------------
// 파괴자
destructor CMXTradeStrategyMOV_N1.Destroy();
begin
  inherited Destroy();
end;
...
// ---------------------------------------------------------------------------
// 계산에 필요한 라인을 생성한다.
procedure CMXTradeStrategyMOV_N1.CreateLineValueSeries;
var
...
begin
  ...
  f_LineValueSeries.m_Name := 'ST-MOV-N1-L1';
  ...
  f_LineValueSeries.m_Name := 'ST-MOV-N1-S1';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
...
// ---------------------------------------------------------------------------
// 계산을 시작한다.
procedure CMXTradeStrategyMOV_N1.Calculate(ARecalculation: Boolean);
var
...
// ---------------------------------------------------------------------------
// 신호를 계산한다.
procedure CMXTradeStrategyMOV_N1.CalulateSignal(p_Begin: Integer; p_End: Integer);
var
  f_Index: Integer;
...
{$REGION 'MA1 < MA3 => 매수'}
          ...
          begin
            ...
            begin
              f_TargetValue0 := f_SourceLineValue0.m_Value[2] - ((f_Tolerance / 100.0) * f_SourceLineValue0.m_Value[2]);
            end
            else
            begin
              f_TargetValue0 := f_SourceLineValue0.m_Value[2] - f_Tolerance;
            end;
          end
          else
          begin
            f_TargetValue0 := f_SourceLineValue0.m_Value[2];
          end;
