...
type
  // ---------------------------------------------------------------------------
  // 이동평균선 비추세 2에 사용하는 보조지표와 신호를 계산하는 클래스
  // ---------------------------------------------------------------------------
  CMXTradeStrategyMOV_N2 = class(CMXTradeStrategy)
  public
    // 생성자
    constructor Create();
...
// ---------------------------------------------------------------------------
// 생성자
constructor CMXTradeStrategyMOV_N2.Create();
begin
  inherited Create();

  CMXTradeStrategyOption.Default_MOV_N2_O1(m_Option);
...
// ---------------------------------------------------------------------------
// 파괴자
destructor CMXTradeStrategyMOV_N2.Destroy();
begin
  inherited Destroy();
end;
...
// ---------------------------------------------------------------------------
// 계산에 필요한 라인을 생성한다.
procedure CMXTradeStrategyMOV_N2.CreateLineValueSeries;
var
...
begin
  ...
  f_LineValueSeries.m_Name := 'ST-MOV-N2-L1';
  f_LineValueSeries.m_LineColors[0] := 30;
  f_LineValueSeries.m_LineColors[1] := 31;
  f_LineValueSeries.m_LineColors[2] := 32;
  ...
  f_LineValueSeries.m_Name := 'ST-MOV-N2-S1';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
...
// ---------------------------------------------------------------------------
// 계산을 시작한다.
procedure CMXTradeStrategyMOV_N2.Calculate(ARecalculation: Boolean);
var
...
{$REGION '첫번째 보조지표를 계산한다'}
  ...
  begin
    ...
    f_LineValueSeries.TS_NAverage(m_Option.GetIntegerValue('LENGTH3'), m_PriceLineValueSeries, 3, 2, f_Begin, f_End);
  end
  ...
  begin
    ...
    f_LineValueSeries.TS_WAverage(m_Option.GetIntegerValue('LENGTH3'), m_PriceLineValueSeries, 3, 2, f_Begin, f_End);
  end
  else
  begin
    ...
    f_LineValueSeries.TS_XAverage(m_Option.GetIntegerValue('LENGTH3'), m_PriceLineValueSeries, 3, 2, f_Begin, f_End);
  end;
{$ENDREGION}
...
// ---------------------------------------------------------------------------
// 신호를 계산한다.
procedure CMXTradeStrategyMOV_N2.CalulateSignal(p_Begin: Integer; p_End: Integer);
var
  f_Index: Integer;
...
{$REGION 'MA1 < MA2 < MA3 => 매수'}
          ...
          begin
            ...
            begin
              ...
              f_TargetValue1 := f_SourceLineValue0.m_Value[2] - ((f_Tolerance / 100.0) * f_SourceLineValue0.m_Value[2]);
            end
            else
            begin
              f_TargetValue0 := f_SourceLineValue0.m_Value[1] - f_Tolerance;
              f_TargetValue1 := f_SourceLineValue0.m_Value[2] - f_Tolerance;
            end;
          end
          else
          begin
            f_TargetValue0 := f_SourceLineValue0.m_Value[1];
            f_TargetValue1 := f_SourceLineValue0.m_Value[2];
          end;

          if (f_SourceLineValue0.m_Value[0] < f_TargetValue0) and (f_SourceLineValue0.m_Value[1] < f_TargetValue1) then
          begin
