...
type
  // RSI 비추세 1에 사용하는 보조지표와 신호를 계산하는 클래스
  CMXTradeStrategyRSI_N1 = class(CMXTradeStrategy)
  public
    // 생성자
    constructor Create();
...
// ---------------------------------------------------------------------------
// 생성자
constructor CMXTradeStrategyRSI_N1.Create();
begin
  inherited Create();
...
// ---------------------------------------------------------------------------
// 파괴자
destructor CMXTradeStrategyRSI_N1.Destroy();
begin
  inherited Destroy();
end;
...
// ---------------------------------------------------------------------------
// 계산에 필요한 라인을 생성한다.
procedure CMXTradeStrategyRSI_N1.CreateLineValueSeries;
var
...
begin
  ...
  // 첫번째 라인
  f_LineValueSeries := Creator_RSI();
  f_LineValueSeries.m_Name := 'ST-RSI-N1-L1';
  ...
  f_LineValueSeries.m_Name := 'ST-RSI-N1-S1';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
...
// ---------------------------------------------------------------------------
// 옵션이 변경되어 재계산이 될 때 화면에 표시한 것들중에 옵션에 관련된 사항들을 적용한다.
procedure CMXTradeStrategyRSI_N1.UpdateVisibleStateOfLineValueSeries;
var
  f_LineValueSeries: CMXTSLineValueSeries;
begin
...
// ---------------------------------------------------------------------------
procedure CMXTradeStrategyRSI_N1.Calculate(ARecalculation: Boolean);
var
...
{$REGION '첫번째 보조지표를 계산한다'}
  ...
  f_LineValueSeries.TS_RSI(m_Option.GetIntegerValue('LENGTH1'), m_PriceLineValueSeries, 3, 0, f_Begin, f_End);
{$ENDREGION}
...
// ---------------------------------------------------------------------------
// 신호를 계산한다.
procedure CMXTradeStrategyRSI_N1.CalulateSignal(p_Begin: Integer; p_End: Integer);
var
  f_Index: Integer;
...
{$ENDREGION}
  f_USE_EXITCONDITON := m_Option.GetBooleanValue('USE_DEFAULT_EXIT_CONDITON');
...
{$REGION '하한선을 상향돌파 => 매수'}
          ...
          begin
            ...
            begin
              f_TargetValue0 := f_DN + ((f_Tolerance / 100.0) * f_DN);
            end
            else
            begin
              f_TargetValue0 := f_DN + f_Tolerance;
            end;
          end
          else
          begin
          ...
          if (f_TargetValue0 >= f_SourceLineValue1.m_Value[0]) AND (f_TargetValue0 < f_SourceLineValue0.m_Value[0]) then
          begin
...
{$REGION '상한선을 하향돌파 => 매도'}
          ...
          begin
            if 0 = f_ToleranceUnit then
            begin
              f_TargetValue0 := f_UP - ((f_Tolerance / 100.0) * f_DN);
            end
            else
            begin
              f_TargetValue0 := f_UP - f_Tolerance;
            end;
          end
          else
          begin
...
{$ENDREGION}
        ...
        if (f_USE_EXITCONDITON) then
        begin
          if (f_SigLineValue0.m_Value[0] = STRATEGY_BUY) then
          begin
{$REGION '하한선을 하향돌파 => 매수청산'}
            if f_UseTolerance then
            begin
              if 0 = f_ToleranceUnit then
              begin
...
{$ENDREGION}
          ...
          if (f_SigLineValue0.m_Value[0] = STRATEGY_SELL) then
          begin
{$REGION '상한선을 상향돌파 => 매도청산'}
            if f_UseTolerance then
            begin
              if 0 = f_ToleranceUnit then
              begin
                f_TargetValue0 := f_UP + ((f_Tolerance / 100.0) * f_UP);
              end
              else
              begin
                f_TargetValue0 := f_UP + f_Tolerance;
