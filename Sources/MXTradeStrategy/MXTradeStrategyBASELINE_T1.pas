unit MXTradeStrategyBASELINE_T1;
...
type
  // ---------------------------------------------------------------------------
  // BASELINE  추세 1에 사용하는 보조지표와 신호를 계산하는 클래스
  // ---------------------------------------------------------------------------
  CMXTradeStrategyBASELINE_T1 = class(CMXTradeStrategy)
  public
    // 생성자
    constructor Create();
...
// ---------------------------------------------------------------------------
// 생성자
constructor CMXTradeStrategyBASELINE_T1.Create();
begin
  inherited Create();

  CMXTradeStrategyOption.Default_BASELINE_T1(m_Option);
...
// ---------------------------------------------------------------------------
// 파괴자
destructor CMXTradeStrategyBASELINE_T1.Destroy();
begin
  inherited Destroy();
end;
...
// ---------------------------------------------------------------------------
// 계산에 필요한 라인을 생성한다.
procedure CMXTradeStrategyBASELINE_T1.CreateLineValueSeries;
var
...
begin
  ...
  f_LineValueSeries.m_Name := 'ST-BASELINE-T1-L1';
  ...
  f_LineValueSeries.m_Name := 'ST-BASELINE-T1-L2';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
  ...
  f_LineValueSeries.m_Name := 'ST-BASELINE-T1-S1';
  f_LineValueSeries.m_TradeStrategy := true;
  f_LineValueSeries.m_TradeStrategyIndex[0] := m_ChartBlockCount;
  f_LineValueSeries.m_TradeStrategyIndex[1] := -1;
  AddLineValueSeries(f_LineValueSeries);
...
// ---------------------------------------------------------------------------
// 옵션이 변경되어 재계산이 될 때 화면에 표시한 것들중에 옵션에 관련된 사항들을 적용한다.
procedure CMXTradeStrategyBASELINE_T1.UpdateVisibleStateOfLineValueSeries;
var
  f_LineValueSeries: CMXTSLineValueSeries;
  f_Index: Integer;
begin
...
// ---------------------------------------------------------------------------
// 계산을 시작한다.
procedure CMXTradeStrategyBASELINE_T1.Calculate(ARecalculation: Boolean);
var
...
{$REGION '첫번째 보조지표를 계산한다'}
  ...
  // 전일종가
  f_LineValueSeries.TS_DayC(m_ChartDataSeries, m_DayChartDataSeries, 1, f_VALUE_TYPE, 0, f_Begin, f_End);
  // 당일시가
  f_LineValueSeries.TS_DayO(m_ChartDataSeries, m_DayChartDataSeries, 0, f_VALUE_TYPE, 1, f_Begin, f_End);
{$ENDREGION}
...
// ---------------------------------------------------------------------------
// 신호를 계산한다.
procedure CMXTradeStrategyBASELINE_T1.CalulateSignal(p_Begin: Integer; p_End: Integer);
var
  f_Index: Integer;
...
{$REGION '보조지표들의 특정을 비교하여 신호 계산'}
  ...
  begin
    ...
    begin
      ...
      if ((f_SourceLineValue0.m_Value[0] <> NOT_VALUE) AND (f_SourceLineValue0.m_Value[1] <> NOT_VALUE) AND
          (f_SourceLineValue1.m_Value[0] <> NOT_VALUE) AND (f_SourceLineValue1.m_Value[1] <> NOT_VALUE) AND
      ...
      begin
        ...
        // (전일종가 < 당일시가) ==> 상승인식
        if (f_SourceLineValue0.m_Value[f_C_1_IDX] < f_SourceLineValue0.m_Value[f_O_0_IDX])
        (* and (f_SourceLineValue0.m_Value[f_L_2_IDX] < f_SourceLineValue0.m_Value[f_O_0_IDX]) *) then
        begin
          if (f_SigLineValue0.m_Value[0] <> STRATEGY_BUY) then
          begin
{$REGION '당일시가를 상향돌파 => 매수'}
            if f_UseTolerance then
            begin
              if 0 = f_ToleranceUnit then
              begin
                f_TargetValue1 := f_SourceLineValue1.m_Value[f_O_0_IDX] +
                    ((f_Tolerance / 100.0) * f_SourceLineValue1.m_Value[f_O_0_IDX]);
                f_TargetValue0 := f_SourceLineValue0.m_Value[f_O_0_IDX] +
                    ((f_Tolerance / 100.0) * f_SourceLineValue0.m_Value[f_O_0_IDX]);
              end
              else
              begin
                f_TargetValue1 := f_SourceLineValue1.m_Value[f_O_0_IDX] + f_Tolerance;
                f_TargetValue0 := f_SourceLineValue0.m_Value[f_O_0_IDX] + f_Tolerance;
              end;
            end
            else
            begin
...
{$ENDREGION}
{$REGION '전일종가를 상향돌파 => 매수'}
            if f_UseTolerance then
            begin
              if 0 = f_ToleranceUnit then
              begin
                f_TargetValue1 := f_SourceLineValue1.m_Value[f_C_1_IDX] +
                    ((f_Tolerance / 100.0) * f_SourceLineValue1.m_Value[f_C_1_IDX]);
                f_TargetValue0 := f_SourceLineValue0.m_Value[f_C_1_IDX] +
                    ((f_Tolerance / 100.0) * f_SourceLineValue0.m_Value[f_C_1_IDX]);
              end
              else
              begin
                f_TargetValue1 := f_SourceLineValue1.m_Value[f_C_1_IDX] + f_Tolerance;
                f_TargetValue0 := f_SourceLineValue0.m_Value[f_C_1_IDX] + f_Tolerance;
              end;
            end
            else
            begin
