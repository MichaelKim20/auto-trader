unit FNNAVAnalManageChartControl;
...
uses
    ...
    FNNAVAnalChartTraceEvent, FNNAVAnalBlockManager, FNNAVAnalChartControlBase;

type
    CFNNAVAnalManageChartControl = class(CFNNAVAnalChartControlBase)
    private
        m_MatrixBlockManager   : CFNNAVAnalBlockManager;
        m_ChartBlockManager   : CFNNAVAnalChartBlockManager;
        m_PriceChartBlock     : CFNNAVAnalChartBlock;
        m_MatrixChartBlock     : CFNNAVAnalChartBlock;
    ...
    public
        ...
        m_MatrixSeries      : CFNNAVAnalLineValueSeries;
    ...
    published
        ...
        property MatrixBlockManager: CFNNAVAnalBlockManager read m_MatrixBlockManager write m_MatrixBlockManager;
...
procedure Register;
begin
  RegisterComponents('ATPackage', [CFNNAVAnalManageChartControl]);
end;

//---------------------------------------------------------------------------
constructor CFNNAVAnalManageChartControl.Create(AOwner: TComponent);
begin
...
//---------------------------------------------------------------------------
destructor CFNNAVAnalManageChartControl.Destroy;
begin
    Finalize;
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.Initialize;
begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.Finalize;
begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.Clear;
begin
    m_ChartDataSeries.Clear;
    if m_PriceSeries <> NIL then m_PriceSeries.Clear;
    if m_MatrixSeries <> NIL then m_MatrixSeries.Clear;
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.WMSize(var Message: TWMSize);
begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.SetColorSetIndex(p_Value:Integer; p_Paint:Boolean = false);
begin
    m_ChartBlockManager.SetColorSetIndex(p_Value, p_Paint);
end;

//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.SetBound(p_Left:Integer; p_Top:Integer; p_Right:Integer; p_Bottom:Integer);
begin
    m_ChartBlockManager.SetBound(p_Left, p_Top, p_Right, p_Bottom);
end;

//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.OnResize(p_Left:Integer; p_Top:Integer; p_Width:Integer; p_Height:Integer; p_Paint:Boolean);
begin
    m_ChartBlockManager.OnResize(p_Left, p_Top, p_Width, p_Height, p_Paint);
end;

//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.RepaintDraw(p_Bitmap:TBitmap32);
begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.Update(p_ChangedDailyGap:Boolean);
var
    nIndex:Integer;
    f_AddCount:Integer;
...
begin
    if not Assigned(m_MatrixBlockManager) then exit;
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.SetChartType(p_Type:Integer; p_Paint:Boolean=false);
var
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.OnControlPaintStage(Sender: TObject; Buffer: TBitmap32; StageNum: Cardinal);
begin
    m_ChartBlockManager.Draw(Buffer);
end;

//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.SetScrollBar(AScrollBar : TScrollBar);
begin
...
//---------------------------------------------------------------------------
function CFNNAVAnalManageChartControl.GetEnableChartControl : Boolean;
begin
...
//---------------------------------------------------------------------------
//스크롤 이벤트
procedure CFNNAVAnalManageChartControl.OnHScroll(Sender: TObject; ScrollCode: TScrollCode; var ScrollPos: integer);
begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.MyScrollMove(Sender: TObject; ScrollCode: TScrollCode; var ScrollPos: integer);
var
    f_OldPoint :Integer;
    f_NewPoint : Integer;
    f_NowPoint : Integer;
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.OnZoomIn;
begin
    if (GetEnableChartControl) then
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.OnZoomOut;
begin
    if (GetEnableChartControl) then
        m_ChartBlockManager.Enlarge(-1, true);
end;
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.OnZoomActual;
begin
    if (GetEnableChartControl) then
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.OnFullEnlarge;
begin
    if (GetEnableChartControl) then
        m_ChartBlockManager.FullEnlarge(true);
end;
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.MouseMoveHandler(Sender: TObject; Shift: TShiftState; X, Y: Integer; Layer: TCustomLayer);
begin
    if (GetEnableChartControl) then
    begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.MouseDownHandler(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer; Layer: TCustomLayer);
begin
    if (GetEnableChartControl) then
    begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.MouseUpHandler(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer; Layer: TCustomLayer);
begin
    if (GetEnableChartControl) then
    begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.SetScale(p_Value:Integer; p_Paint:Boolean=false);
begin
    m_Scale := p_Value;
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.SetTraceVisible(p_Value:Boolean; p_Paint:Boolean = false);
begin
    m_ChartBlockManager.SetTraceVisible(p_Value, p_Paint);
end;

//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.DrawTrace(p_ChartIndex:Integer; p_ValueX:Double; p_ValueY:Double);
begin
...
//---------------------------------------------------------------------------
procedure CFNNAVAnalManageChartControl.GetChartMaxMin(var p_Min, p_Max:Double);
begin
