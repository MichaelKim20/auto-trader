unit FNNAVAnalChartBlockManager;
...
uses
  ...
  FNNAVAnalChartDataSeries, FNNAVAnalColorSet, FNNAVAnalConst, FNNAVAnalMaxMin,
  FNQueue, FNThread, SyncObjs, Graphics,
  FNNAVAnalChartData, FNNAVAnalLineValueSeries,
  FNNAVAnalChartBlock, FNNAVAnalPosInfo, FNNAVAnalChartTraceEvent,
  FNNAVAnalPosValue;

type
  pTPoint = ^TPoint;

  CFNNAVAnalChartBlockManager = class(TObject)
  private
    m_ChartArray: TList;
    m_Scale: Integer;
    ...
    m_LastTRInfo: CFNNAVAnalPosInfo;
  ...
  public
    ...
    function GetColorSet: CFNNAVAnalColorSet;
    ...
    procedure LayOut;
    function AddChart(p_Name: String): CFNNAVAnalChartBlock;
    function InsertChart(p_Name: String): CFNNAVAnalChartBlock;
    procedure DeleteChart(p_Name: String);
    procedure DeleteChartAll;
    procedure ClearChartAll;
    function FindChart(p_Name: String): CFNNAVAnalChartBlock;
    procedure Clear;
    ...
    function GetMaxMin: CFNNAVAnalMaxMin;
    ...
    procedure ClickPos(p_TRInfo: CFNNAVAnalPosInfo);
    procedure TraceXYOnSometime(p_TRInfo: CFNNAVAnalPosInfo);

    procedure TraceXYOnAnytime(p_TRInfo: CFNNAVAnalPosInfo);
    ...
    procedure DrawTracePannel(p_TRInfo: CFNNAVAnalPosInfo);
...
uses
  FNGlobal, FNNAVAnalChartControlBase, FNNAVAnalChartDefine;

// ---------------------------------------------------------------------------
constructor CFNNAVAnalChartBlockManager.Create;
begin
  inherited Create;

  m_ChartArray := TList.Create;
  m_ChartType := CFNNAVAnalConst.CHART_NORMAL;
  m_Scale := 1;
  m_ColorSet := CFNNAVAnalColorSet.Create;
  ...
  m_LastTRInfo := CFNNAVAnalPosInfo.Create;
...
// ---------------------------------------------------------------------------
destructor CFNNAVAnalChartBlockManager.Destroy;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetLayer;
begin
  m_OverLayer := TBitmapLayer.Create(CFNNAVAnalChartControlBase(m_ChartControl).Layers);
  m_OverLayer.Bitmap.DrawMode := dmBlend;
  m_OverLayer.OnPaint := OnPaintOverLayerManager;

  m_SignalLayer := TBitmapLayer.Create(CFNNAVAnalChartControlBase(m_ChartControl).Layers);
  m_SignalLayer.Bitmap.DrawMode := dmBlend;
  m_SignalLayer.Bitmap.CombineMode := cmMerge;

  m_TraceLayer := TBitmapLayer.Create(CFNNAVAnalChartControlBase(m_ChartControl).Layers);
  m_TraceLayer.Bitmap.DrawMode := dmBlend;

  m_PosValueLayer := TBitmapLayer.Create(CFNNAVAnalChartControlBase(m_ChartControl).Layers);
  m_PosValueLayer.Bitmap.DrawMode := dmBlend;

  m_LabelLayer := TBitmapLayer.Create(CFNNAVAnalChartControlBase(m_ChartControl).Layers);
  m_LabelLayer.Bitmap.DrawMode := dmBlend;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetColorSetIndex(p_Value: Integer; p_Paint: Boolean = false);
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetDisplayTypeOfTrade(p_Value: Integer);
var
  f_Index: Integer;
...
begin
  ...
  begin
    f_ChartBlock := CFNNAVAnalChartBlock(m_ChartArray.Items[f_Index]);
...
// ---------------------------------------------------------------------------
function CFNNAVAnalChartBlockManager.GetColorSet: CFNNAVAnalColorSet;
begin
  Result := m_ColorSet;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetChartControl(p_ChartControl: TObject);
begin
  m_ChartControl := p_ChartControl;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetBound(p_Left: Integer; p_Top: Integer; p_Width: Integer; p_Height: Integer);
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.OnResize(p_Left: Integer; p_Top: Integer; p_Width: Integer; p_Height: Integer; p_Paint: Boolean);
begin
  m_BoundRect := TFNGlobal.Rect2(p_Left, p_Top, p_Width, p_Height);
  if (p_Paint) then
  begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.LayOut;
var
  f_ChartBlock: CFNNAVAnalChartBlock;
...
begin
  ...
  begin
    f_ChartBlock := CFNNAVAnalChartBlock(m_ChartArray.Items[0]);
  ...
  begin
    ...
    begin
      ...
      if (f_ChartIndex = m_ChartArray.Count - 1) then
        f_Bottom := f_SpaceRect.bottom - CFNNAVAnalConst.CHART_INTERHEIGHT
      else
        f_Bottom := f_Top + Round(TFNGlobal.RectToHeight(f_SpaceRect) / (m_ChartArray.Count - 1)) - CFNNAVAnalConst.CHART_INTERHEIGHT;

      f_ChartBlock := CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]);
      f_ChartBlock.SetBound(f_Left, f_Top, f_Right, f_Bottom);

      f_Bottom := f_Bottom + CFNNAVAnalConst.CHART_INTERHEIGHT;
    end;
  end;
...
// ---------------------------------------------------------------------------
function CFNNAVAnalChartBlockManager.AddChart(p_Name: String): CFNNAVAnalChartBlock;
var
  f_ChartBlock: CFNNAVAnalChartBlock;
  f_ChartIndex: Integer;
begin
  f_ChartBlock := CFNNAVAnalChartBlock.Create;
  ...
  begin
    CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).m_ChartIndex := f_ChartIndex;
  end;
...
// ---------------------------------------------------------------------------
function CFNNAVAnalChartBlockManager.InsertChart(p_Name: String): CFNNAVAnalChartBlock;
var
  f_ChartBlock: CFNNAVAnalChartBlock;
  f_ChartIndex: Integer;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.DeleteChart(p_Name: String);
var
  f_Index: Integer;
  f_ChartIndex: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
  f_FindChart: CFNNAVAnalChartBlock;
begin
  f_FindChart := NIL;
  for f_Index := 0 to m_ChartArray.Count - 1 do
  begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.DeleteChartAll;
var
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.ClearChartAll;
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
function CFNNAVAnalChartBlockManager.FindChart(p_Name: String): CFNNAVAnalChartBlock;
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
  f_FindChart: CFNNAVAnalChartBlock;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.Clear;
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.ClearTraceLayer;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.ClearActiveLayer;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.ClearPosValueLayer;
begin
...
procedure CFNNAVAnalChartBlockManager.ClearLabelLayer;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.ClearSignalLayer;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.Draw(p_Bitmap: TBitmap32);
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
  try
    ...
    begin
      p_Bitmap.Clear(m_ColorSet.m_Color[CFNNAVAnalColorSet.CHART_BACKGROUND_COLOR]);
    end;
  finally
  end;
end;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.RePaint;
begin
  try
    Clear;
    CFNNAVAnalChartControlBase(m_ChartControl).Invalidate;
  finally
  end;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetChartDataSeries(p_ChartDataSeries: CFNNAVAnalChartDataSeries);
var
  f_Index: Integer;
begin
  ...
  for f_Index := 0 to m_ChartArray.Count - 1 do
  begin
    CFNNAVAnalChartBlock(m_ChartArray.Items[f_Index]).m_ChartDataSeries := m_ChartDataSeries;
  end;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetScale(p_Scale: Integer);
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
function CFNNAVAnalChartBlockManager.GetMaxMin: CFNNAVAnalMaxMin;
begin
  Result := CFNNAVAnalChartBlock(m_ChartArray.Items[0]).m_MaxMin;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.FirstEnlarge(p_Paint: Boolean);
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.FullEnlarge(p_Paint: Boolean);
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.EnlargeValue(p_Paint: Boolean; p_Value: Integer);
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.Enlarge(p_Rratio: Integer; p_Paint: Boolean = false);
var
  f_PriceChart: CFNNAVAnalChartBlock;
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
  f_PriceChart := CFNNAVAnalChartBlock(m_ChartArray.Items[0]);
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.RangeEnlarge(f_XMin: Double = -1000000; f_XMax: Double = -1000000; p_SetScrollBarPropertis: Boolean = true; p_Notify: Boolean = true);
var
  f_Index: Integer;
  f_SmallSize: Integer;
begin
  ...
  begin
    if (f_XMin = -1000000) then
      f_XMin := CFNNAVAnalChartBlock(m_ChartArray.Items[0]).m_MaxMin.m_XMin;

    if (f_XMax = -1000000) then
      f_XMax := CFNNAVAnalChartBlock(m_ChartArray.Items[0]).m_MaxMin.m_XMax;
  end;

  for f_Index := 0 to m_ChartArray.Count - 1 do
  begin
    CFNNAVAnalChartBlock(m_ChartArray.Items[f_Index]).RangeEnlarge(f_XMin, f_XMax, true);
  end;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetScrollBar(p_ScrollBar: TScrollBar);
begin
  m_ScrollBar := p_ScrollBar;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.OnHScroll(Sender: TObject; ScrollCode: TScrollCode; var ScrollPos: Integer);
var
  f_ChartBlock: CFNNAVAnalChartBlock;
  ...
  f_ChartData1: CFNNAVAnalChartData;
  f_ChartData0: CFNNAVAnalChartData;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetScrollBarPosition;
var
  f_ChartBlock: CFNNAVAnalChartBlock;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.OnMouseDown(p_X: Integer; p_Y: Integer);
var
  m_EventRect: TRect;
...
begin
  ...
  for f_ChartIndex := 0 to m_ChartArray.Count - 1 do
  begin
    m_EventRect := TFNGlobal.Rect2(CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).m_BoundRect.left, CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).m_BoundRect.top,
      TFNGlobal.RectToWidth(CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).m_BoundRect) - 50, TFNGlobal.RectToHeight(CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).m_BoundRect));
  ...
  if (f_MouseChartIndex >= 0) then
    CFNNAVAnalChartBlock(m_ChartArray.Items[f_MouseChartIndex]).OnMouseDown(p_X, p_Y);
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.OnMouseUp(p_X: Integer; p_Y: Integer);
var
  m_EventRect: TRect;
...
begin
  ...
  for f_ChartIndex := 0 to m_ChartArray.Count - 1 do
  begin
    if (CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).m_CaptureMouse) then
    begin
      f_MouseChartIndex := f_ChartIndex;
      f_CaptureMouse := true;
      break;
  ...
  if (f_MouseChartIndex >= 0) then
    CFNNAVAnalChartBlock(m_ChartArray.Items[f_MouseChartIndex]).OnMouseUp(p_X, p_Y);
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.OnMouseMove(p_X: Integer; p_Y: Integer);
var
  f_pt: TPoint;
  m_EventRect: TRect;
  f_ChartIndex, f_MouseChartIndex: Integer;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (PtInRect(m_EventRect, Point(p_X, p_Y))) then
        begin
          CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).OnMouseMoveOnAnytime(p_X, p_Y);
          CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).OnMouseMoveSometime(p_X, p_Y);
        end;
      end;
    end;
  except
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.DrawTraceCaption(p_ValueX: Integer);
var
  f_Index: Integer;
begin
  ClearLabelLayer;
  ...
  for f_Index := 0 to m_ChartArray.Count - 1 do
  begin
    CFNNAVAnalChartBlock(m_ChartArray.Items[f_Index]).DrawCaption(p_ValueX);
  end;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.TraceXYOnSometime(p_TRInfo: CFNNAVAnalPosInfo);
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.ClickPos(p_TRInfo: CFNNAVAnalPosInfo);
var
  f_Index: Integer;
...
begin
  ...
  if Assigned(CFNNAVAnalChartControlBase(m_ChartControl).OnClickPos) then
  begin
    CFNNAVAnalChartControlBase(m_ChartControl).OnClickPos(p_TRInfo.m_ChartIndex, p_TRInfo.m_ValueX, p_TRInfo.m_ValueY);
  end;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.TraceXYOnAnytime(p_TRInfo: CFNNAVAnalPosInfo);
var
  f_Index: Integer;
  f_ChartData: CFNNAVAnalChartData;
begin
  ...
  if Assigned(CFNNAVAnalChartControlBase(m_ChartControl).OnChartTraceChange) then
  begin
    CFNNAVAnalChartControlBase(m_ChartControl).OnChartTraceChange(p_TRInfo.m_ChartIndex, p_TRInfo.m_ValueX, p_TRInfo.m_ValueY);
  end;
end;

procedure CFNNAVAnalChartBlockManager.DrawTrace(p_ChartIndex: Integer; p_ValueX: Double; p_ValueY: Double);
var
  f_pt: TPoint;
  m_EventRect: TRect;
  f_ChartIndex, f_MouseChartIndex: Integer;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.TraceOnLastTime;
var
  f_Index: Integer;
  f_ChartData: CFNNAVAnalChartData;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
  ...
  try
    ...
    begin
      ...
      for f_Index := 0 to m_ChartArray.Count - 1 do
      begin
        CFNNAVAnalChartBlock(m_ChartArray.Items[f_Index]).DrawTrace(m_LastTRInfo);
        CFNNAVAnalChartBlock(m_ChartArray.Items[f_Index]).InitCaption;
      end;
    end;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetXExtraGap(p_X: Integer);
begin

end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.ClearTrace;
begin
  ClearTraceLayer;
  ClearPosValueLayer;
end;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetVisibleXLabel(p_Value: Boolean);
var
  f_Index: Integer;
  f_ChartBlock: CFNNAVAnalChartBlock;
begin
...
// ---------------------------------------------------------------------------
function CFNNAVAnalChartBlockManager.GetVisibleXLabel(): Boolean;
begin
  Result := m_XLabel;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetTraceVisible(p_Value: Boolean; p_Paint: Boolean = false);
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetUnitType(AValue: Integer);
begin
  m_UnitType := AValue;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.SetUseOPSPrice(AValue: Boolean);
begin
  m_UseOPSPrice := AValue;
end;

// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.DrawPosValueFrame(p_X: Integer; p_Y: Integer; p_Width: Integer; p_Height: Integer; p_CX: Integer; p_CY: Integer; p_Direct: Integer);
var
  PenColor: TColor32;
  FillColor: TColor32;
begin
  ...
  PenColor := m_ColorSet.m_Color[CFNNAVAnalColorSet.TRACE_VALUE_LINE_COLOR];
  PenColor := SetAlpha(PenColor, TFNGlobal.GetAlphaValue(100));
  FillColor := m_ColorSet.m_Color[CFNNAVAnalColorSet.TRACE_VALUE_FILLED_COLOR];
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.DrawTraceXValueFrame(p_X: Integer; p_Y: Integer; p_Width: Integer; p_Height: Integer);
var
  PenColor: TColor32;
  FillColor: TColor32;
begin
  ...
  PenColor := SetAlpha(PenColor, TFNGlobal.GetAlphaValue(100));
  FillColor := m_ColorSet.m_Color[CFNNAVAnalColorSet.TRACE_XY_FILLED_COLOR];
  FillColor := SetAlpha(FillColor, TFNGlobal.GetAlphaValue(100));
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.DrawTraceYValueFrame(p_X: Integer; p_Y: Integer; p_Width: Integer; p_Height: Integer);
var
  PenColor: TColor32;
  FillColor: TColor32;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.DrawTracePannel(p_TRInfo: CFNNAVAnalPosInfo);
var
  f_Index, f_LineIndex, f_LineCount: Integer;
  f_ValueArray: TList;
  p_X, p_Y, p_W, p_H: Integer;
  p_ChartData: CFNNAVAnalChartData;
  p_PosValue: CFNNAVAnalPosValue;
  f_Value: String;
...
begin
  ...
  begin
    p_ChartData := CFNNAVAnalChartData(m_ChartDataSeries.m_Items.Items[Math.floor(p_TRInfo.m_ValueX)]);
  ...
  m_PosValueLayer.Bitmap.Font.Name := CFNNAVAnalColorSet.NUMBER2_FONT_FAMILY;
  m_PosValueLayer.Bitmap.Font.Size := CFNNAVAnalColorSet.NUMBER2_FONT_SMALLSIZE2;
  m_PosValueLayer.Bitmap.Font.Style := [];

  FontColor := m_ColorSet.m_Color[CFNNAVAnalColorSet.TRACE_XY_TEXT_COLOR];
  ...
  begin
    ...
    p_H := CFNNAVAnalConst.CHART_DRAW_XLABEL_HEIGHT;
    p_X := Math.floor(p_TRInfo.m_WindowX - (p_W / 2));
    p_Y := CFNNAVAnalChartBlock(m_ChartArray.Items[0]).m_AxisRect.top + RectHeight(CFNNAVAnalChartBlock(m_ChartArray.Items[0]).m_AxisRect);
    ...
    begin
      f_ChartBlock := CFNNAVAnalChartBlock(m_ChartArray.Items[p_TRInfo.m_ChartIndex]);
      ...
      begin
        ...
        f_YLabelWidth := f_ChartBlock.m_PaddingRight + CFNNAVAnalConst.CHART_DRAW_YLABEL_WIDTH - 2;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalChartBlockManager.OnMouseLeave;
begin
  ClearActiveLayer;
end;

procedure CFNNAVAnalChartBlockManager.OnPaintOverLayerManager(Sender: TObject; Buffer: TBitmap32);
var
  f_ChartIndex: Integer;
  f_ClipRect: TRect;
begin
  try
    for f_ChartIndex := 0 to m_ChartArray.Count - 1 do
    begin
      f_ClipRect := Rect(m_BoundRect.left, m_BoundRect.top, CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).m_AxisRect.right - 1, m_BoundRect.bottom);
      Buffer.ResetClipRect;
      Buffer.ClipRect := f_ClipRect;

      CFNNAVAnalChartBlock(m_ChartArray.Items[f_ChartIndex]).OnPaintOverLayer(Sender, Buffer);
