...
type
  CFNNAVAnalColorSet = class(TObject)
  public const
...
uses
  FNGlobal, FNNAVAnalConst;

// ---------------------------------------------------------------------------
constructor CFNNAVAnalColorSet.Create;
begin
  inherited Create;
...
// ---------------------------------------------------------------------------
destructor CFNNAVAnalColorSet.Destroy;
begin
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalColorSet.Initialize;
var
  nIndex: Integer;
begin
  m_ColorSetIndex := 1;
...
// ---------------------------------------------------------------------------
procedure CFNNAVAnalColorSet.SetColorSetIndex(p_Value: Integer);
begin
  if (p_Value = CFNNAVAnalConst.COLOR_SET_BLACK) then
  begin
    m_Color[0] := $00F08080;
    m_Color[1] := $008080F0;
    m_Color[2] := $0080F080;
    ...
    m_Alpha[ALPHA_LIGHT_LINE] := TFNGlobal.GetAlphaValue(20);
  ...
  else if (p_Value = CFNNAVAnalConst.COLOR_SET_WHITE) then
  begin
    m_Color[0] := $00C18A60;
    m_Color[1] := $00DA568B;
    m_Color[2] := $00AF7F08;
