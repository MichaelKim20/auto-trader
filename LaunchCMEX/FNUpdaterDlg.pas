...
var
    ...
    OPS_VERSION_INFO_URL : String;

implementation
uses
    FNGlobalVariable, FNGlobal, FNDefine, FNCMVariable, MXVariable, FNRegistry;
{$R *.dfm}
...
var
    f_Idx : Integer;
begin
    Self.Caption            := '프로그램 다운로드';
    m_lbTitle.Caption       := '프로그램을 실행을 위해 준비를 합니다. ';
...
begin
    ...
    g_Version := f_Registry.ReadString('install', 'Version', '1.000');
    f_Registry.Free;
    ...
    m_VersionCheckURL   := OPS_VERSION_INFO_URL + '?' + 's=' + g_SecCode;
end;
...
begin
    ...
    try
        m_LocalFilePath := ExtractFilePath(ParamStr(0)) + g_ApplicationName + '.exe';
...
begin
    ...
    begin
        ...
        f_Result := ShellExecute(Handle,'open', PWideChar(g_ApplicationName + '.exe'), PWideChar(f_PARAM), '', SW_SHOWNORMAL);

        f_Registry := CFNRegistry.Create(self);
        f_Registry.Company := g_CompanyName;
        f_Registry.ApplicationName := g_ApplicationName;
        f_Registry.WriteString('install', 'Version', m_Version);
        f_Registry.Free;
        ...
        begin
            ...
            try
                ...
                begin
                    ...
                    if Length(f_Line) < 3 then continue;

                    if f_LineIndex = 0 then
                    begin
                        m_Version := f_Line;
                    end else
                    if f_LineIndex = 1 then
                    begin
                        m_ServerFilePath := f_Line;
                    end else
                    if f_LineIndex = 2 then
                    begin
...
begin
    if CompareText(m_Version, g_Version) > 0 then
    begin
        Result := TRUE
    end else
    begin
